...
var allSites = JSON.stringify([
    ...
    {
        SiteID: "617",
        SiteName: "Credit Libanais",
        SiteCountry: "Lebanon",
        SiteCountryCode: "2"
    },
    {
        SiteID: "601",
        SiteName: "Delta Trading Co.",
        SiteCountry: "Lebanon",
        SiteCountryCode: "2"
    },
    {
        SiteID: "618",
        SiteName: "Hopital Albert Haykel",
        SiteCountry: "Lebanon",
        SiteCountryCode: "2"
    },
    {
    ...
    {
        SiteID: "577",
        SiteName: "Red Cross Hazmieh",
        SiteCountry: "lebanon",
        SiteCountryCode: "2"
    },
    {
        SiteID: "564",
        SiteName: "Red Cross Spears",
        SiteCountry: "lebanon",
        SiteCountryCode: "2"
    },
    {
...
QUnit.test("Next Button Test", function(assert) {
    ...
    function test_next(next_range) {
        ...
        assert.equal(
            $("#next").text(),
            next_range,
            "The result should be " + next_range
        );
    }

    var next_date_range =
        '{"dateEarly":"2008-02-03T03:56:36.000Z","dateFrom":"2008-01-04T03:56:36.000Z"}';
    if (earliestDate) {
...
QUnit.test("Previous Button Test", function(assert) {
    ...
    function test_previous(previous_range) {
        assert.equal(
            JSON.stringify(previousDateButton()),
            JSON.stringify(previous_range),
            "The result should be " + JSON.stringify(previous_range)
        );
    }
...
QUnit.test("First Button Test", function(assert) {
    ...
    function test_first(previous_range) {
        assert.equal(
            JSON.stringify(firstDateButton()),
            JSON.stringify(previous_range),
            "The result should be " + JSON.stringify(previous_range)
        );
    }
...
QUnit.test("Insights Test", function(assert) {
    $("#qunit-fixture").append(
        '<div id="catDiv"><ul><div id="issues"></div></ul></div>'
    );
    ...
    var data = [
        {
            ...
            avatar:
                "https://lh3.googleusercontent.com/fPtXKFBuMvx7FaUz2J8S9CMJWFU7CYiCB4x9OtZcv3EjxTniJ-F3PXUmUjuqfaH39VTk3siPODA31w=w812-h811-rw-no"
        },
        {
            ...
            TotalDifficulties: 346,
            avatar:
                "https://lh3.googleusercontent.com/Z0EzEvOXm-KCY27xigj5fBoSt0hFGIygw4iZ2mfbSZd665LRF7yHZMvU5XbgtVWL2RnivFnR7GbKuQ=s718-rw-no"
        },
        {
            ...
            TotalDifficulties: 346,
            avatar:
                "https://lh3.googleusercontent.com/lPkdIgRAi0YaOEWmo1tkSRK0EAV0SprkCiebyPcYmeOth2ee2G0DJAFqQQ7P2vx6bBCXZEZgHfGNhg=w1496-h1500-rw-no"
        }
    ];
...
QUnit.test("New Ticket Test", function(assert) {
    ...
    function fillModal(sites, countries) {
        ...
        for (var _i = 0; _i < countries.length; _i++) {
            $("#country").append(
                '<option id="' +
                countries[_i].CountryID +
                '" value="' +
                countries[_i].CountryName +
                '">' +
                countries[_i].CountryName +
                "</option>"
            );
    ...
    function test_buttonSubmit(expected_data) {
        ...
        assert.equal(
            ...
            expected_data,
            "The expected form result is " + expected_data
        );
    }

    var expected_data =
        '{"title":"test","description":"test","product":"test","serial":"test",' +
...
QUnit.test("Latest Ticket Test", function(assert) {
    $("#qunit-fixture").append(
        '<table><tbody id="ticketsTable"></tbody></table>'
    );
    ...
    function test_latestTickets(data, expected_data, type) {
        ...
        if (type === "date") {
            ...
            assert.equal(
                test,
                expected_data,
                "The result should be " + expected_data
            );
        } else {
            var _test = $(".test1")
                .first()[0]
                .innerHTML.substring(25, 32);
            assert.equal(
                _test,
                expected_data,
                "The badge should be " + expected_data
            );
        }
    }
...
QUnit.test("Vul Table Test", function(assert) {
    $("#qunit-fixture").append(
        '<table id="vulTable"><tbody><tr class="rowVul"></tr></tbody></table>'
    );

    function test_vulTable(data, expected_data) {
        vulTable(data);
        var test = $(".rowVul:first .progress-bar").css("width");
        assert.equal(test, expected_data, "The result should be " + expected_data);
    }
...
QUnit.test("Src Table Test", function(assert) {
    $("#qunit-fixture").append(
        '<div id="sources"><table id="sourcesTable"><tbody><tr class="rowClassSrcs"></tr></tbody></table></div>'
    );

    function test_srcTable(data, expected_data) {
        srcsTable(data, 256500000, 250);
        var test = $(".spec:first").css("width");
        //console.log(test);
        assert.equal(test, expected_data, "The result should be " + expected_data);
    }
...
QUnit.test("Delay Graph Test", function(assert) {
    $("#qunit-fixture").append(
        '<div id="delayTable"><div id="delay"><div id="delayGraph"></div><canvas id="delayCanvas"></canvas></div></div>'
    );
